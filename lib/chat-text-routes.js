...
function callAgent(io)
{  
  io.of('/start-chat').on('connection', function(socket){
  console.log('start chat connect');
  socket.on('start chat disconnect', function(){    
    console.log('user disconnected');
    });
  });  

  io.of('/start-chat').emit('message', 'Wannt chat beeatch');   
  sleep(5000);
}
